...
export async function fetchJuizados(req, res) {
    try {
        const [juizados] = await db.query(`
            SELECT j.juizado_id, j.comarca_id, j.nome_juizado, c.nome_comarca 
            FROM juizado AS j
...
export async function addJuizado(req, res) {
    const { comarca_id, nome_juizado } = req.body;
    try {
        const result = await db.query(`
            INSERT INTO juizado (comarca_id, nome_juizado) 
            VALUES (?, ?)
        `, [comarca_id, nome_juizado]);
...
export async function updateJuizado(req, res) {
    ...
    try {
        await db.query(`
            UPDATE juizado 
            SET comarca_id = ?, nome_juizado = ? 
            WHERE juizado_id = ?
        `, [comarca_id, nome_juizado, id]);
